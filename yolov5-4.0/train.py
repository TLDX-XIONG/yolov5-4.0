...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/UAV.yaml', help='data.yaml path') # 数据集配置信息
    ...
    parser.add_argument('--resume', nargs='?', const=True, default=False, help='resume most recent training') # 接着上次训练
    parser.add_argument('--nosave', action='store_true', help='only save final checkpoint') # 
    parser.add_argument('--notest', action='store_true', help='only test final epoch') # 只对最后一个 epoch 进行模型测试
    parser.add_argument('--noautoanchor', action='store_true', help='disable autoanchor check') # 不自动计算锚框
    parser.add_argument('--evolve', action='store_true', help='evolve hyperparameters') # 进行超参数进化
    parser.add_argument('--bucket', type=str, default='', help='gsutil bucket') # 
    parser.add_argument('--cache-images', action='store_true', help='cache images for faster training') # 缓存图片(一次性写入内存)
    parser.add_argument('--image-weights', action='store_true', help='use weighted image selection for training') # 使用图片权重策略选择采样图片
    parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu') # 训练设备
    parser.add_argument('--multi-scale', action='store_true', help='vary img-size +/- 50%%') # 使用多尺度图片
    parser.add_argument('--single-cls', action='store_true', help='train multi-class data as single-class') # 单类
    parser.add_argument('--adam', action='store_true', help='use torch.optim.Adam() optimizer') # 使用 Adam 优化器
    parser.add_argument('--sync-bn', action='store_true', help='use SyncBatchNorm, only available in DDP mode') 
    parser.add_argument('--local_rank', type=int, default=-1, help='DDP parameter, do not modify')
    parser.add_argument('--log-imgs', type=int, default=16, help='number of images for W&B logging, max 100')
    parser.add_argument('--log-artifacts', action='store_true', help='log artifacts, i.e. final trained model')
    parser.add_argument('--workers', type=int, default=0, help='maximum number of dataloader workers') # 进程数
    parser.add_argument('--project', default='runs/train', help='save to project/name') # 保存输出文件路径
    parser.add_argument('--name', default='exp', help='save to project/name') # 保存输出文件文件夹名称

...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            correct = torch.zeros(pred.shape[0], niou, dtype=torch.bool, device=device) # correct 记录 true postive
            if nl:
                ...
                for cls in torch.unique(tcls_tensor):
                    ...
                    if pi.shape[0]:
                        ...
                        # 计算预测框与标签框的 iou 值，并选出最大的 ious(即哪个标签框与预测框最匹配)，i 为标签框对应索引
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='runs/train/exp/weights/best.pt', help='model.pt path(s)') # 权重文件路径
    parser.add_argument('--data', type=str, default='data/UAV.yaml', help='*.data path') # 测试的数据集或者配置文件
    parser.add_argument('--batch-size', type=int, default=16, help='size of each image batch') # batchsize
    parser.add_argument('--img-size', type=int, default=960, help='inference size (pixels)') # 输入模型的图片大小格式
    ...
    parser.add_argument('--task', default='test', help="'val', 'test', 'study'") # 当前任务
